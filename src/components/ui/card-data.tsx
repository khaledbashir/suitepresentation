          ...
          {safeOptions.map((card) => (
            <div key={`${id}-${card.id}`} className="relative group">
              ...
              {(card.description || card.url) && (
                <div className="absolute z-10 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-opacity bottom-full left-0 mb-2 px-3 py-2 bg-popover text-popover-foreground text-xs rounded shadow-lg max-w-xs">
                  <div className="relative">
                    ...
                    {/* Tooltip arrow */}
                    <div className="absolute bottom-0 left-3 -mb-2 transform translate-x-1/2 rotate-45 w-2 h-2 bg-popover"></div>
                  </div>
                </div>
              )}
            </div>
