  ...
  variants: {
    variant: {
      ...
      solid: [
        "[&>div]:bg-background",
        "[&>div]:border-0",
        "[&>div]:shadow-xl [&>div]:shadow-black/50",
...
>(({ className, children, ...props }, ref) => {
  ...
  const buttonClasses = cn(
    "w-10 h-10 bg-accent hover:bg-accent/80 text-text-primary rounded-lg disabled:opacity-50 flex items-center justify-center enabled:cursor-pointer",
    className,
  );
...
/**
 * Icon component for MCP (Model Context Protocol) configuration.
 * @internal
 */
const MCPIcon = () => {
  return (
      ...
      {images.map((image: StagedImage) => (
        <div key={image.id} className="relative group flex-shrink-0 w-20 h-20">
          ...
          <button
            ...
            className="absolute -top-2 -right-2 w-5 h-5 bg-bg-secondary border border-border text-text-secondary hover:bg-bg-tertiary hover:text-text-primary rounded-full flex items-center justify-center transition-colors shadow-sm z-10"
