...
const ToolcallInfo = React.forwardRef<HTMLDivElement, ToolcallInfoProps>(
  ({ className, markdown = true, ...props }, ref) => {
    ...
    const openPreview = useArtifactsStore((s) => s.openPreview);
    ...
    // Try to extract an artifactId from the associated tool response content
    const artifactIdFromResponse = React.useMemo(() => {
      const content = associatedToolResponse?.content;
      ...
      try {
        ...
        if (parsed && typeof parsed === "object" && typeof parsed.artifactId === "string") {
          return parsed.artifactId as string;
        }
      } catch {
    ...
    // Auto-expand when we have an artifact to preview and the tool call is done
    React.useEffect(() => {
      if (!isLoading && artifactIdFromResponse) {
        setIsExpanded(true);
      }
    }, [artifactIdFromResponse, isLoading]);
      ...
      >
        <div className="flex flex-col w-full">
          <button
            ...
            className={cn(
              "flex items-center gap-1 cursor-pointer hover:bg-accent rounded-md p-1 select-none w-fit",
            )}
          >
            {hasToolError ? (
            ...
            {associatedToolResponse && (
              <div className="pl-2">
                ...
                <div>
                  ...
                  {artifactIdFromResponse ? (
                    <div className="pt-2">
                      <button
                        type="button"
                        onClick={() => openPreview(artifactIdFromResponse)}
